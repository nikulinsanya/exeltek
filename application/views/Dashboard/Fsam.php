...
<?php
    $values = array(
        'total' => array_sum(array_column($fsam, 'total')),
        'tested' => array_sum(array_column($fsam, 'tested')),
        'built' => array_sum(array_column($fsam, 'built')),
    );
?>
<div class="col-xs-12">
...
<?php foreach ($fsam as $key => $values) if ($values['total']):?>
    <div class="col-xs-12 progress-row">
        <?php $tested = $values['tested'] ? round($values['tested'] / $values['total'] * 100): 0;?>
        <?php $built = $values['built'] ? round($values['built'] / $values['total'] * 100) : 0;?>
        <div class="control-label">
            <strong>
                <a href="<?=URL::base()?>dashboard/fda<?=URL::query(array('fsam' => $key))?>"><?=$key?>: <?=$values['total']?> ticket(s).</a>
            </strong>
            <div class="progress-label">
                <?php if ($values['tested']):?>
